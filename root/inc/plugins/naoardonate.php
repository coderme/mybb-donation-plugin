 ...
 elseif ($db->table_exists('naoardonate') ) {
  ...
  elseif ( $db->field_exists('payment_method', 'naoardonate') ) {
	...
	if($db->num_rows($query) > 0):
		...
		$db->update_query('settinggroups', array('title' => 'Naoar Donation', 'name' => 'naoardonate'), "gid='{$gid}'");
	else:
...
<input type=\"radio\" name=\"upsetting[{$setting[name]}]\" value=\"0\" class=\"radio_input radio_off naoardonate_settings_onoff\" id=\"naoardonate_off\"  " . ($setting[\'value\'] == 0 ? "checked=\"checked\"" : "" ) . " />' . $lang->no . '</label>'),
		'value' => "$naoardonate_onoff",
		'disporder' => 0,
		'gid' => $gid
	);


	if($mybb->settings['naoardonate_ebank'])
	{
...
<label onclick=\"t_onchange(\'naoardonate_lr\',\'payment_method_lr\');\"  for=\"naoardonate_lr\"><input type=\"checkbox\" name=\"upsetting[naoardonate_payment_method][]\" id=\"naoardonate_lr\" value=\"LibertyReserve\" ".(strpos($setting[\'value\'],\'LibertyReserve\') !== false? "checked=\"checked\"" : "" ). "> LibertyReserve <a href=\"https://www.libertyreserve.com/en/registration\" title=\"'
. $lang->sprintf($lang->naoardonate_settings_get_payment_method_account, 'LibertyReserve') . '\" target=\"_blank\"><img src=\"./../images/naoar/oh.png\"  alt=\"'
. $lang->sprintf($lang->naoardonate_settings_get_payment_method_account, 'LibertyReserve') . '\" style=\"vertical-align:middle;border:0;width:13px;height:13px\"/></a></label><br />
<label onclick=\"t_onchange(\'naoardonate_sk\',\'payment_method_sk\');\" for=\"naoardonate_sk\"><input type=\"checkbox\" name=\"upsetting[naoardonate_payment_method][]\" id=\"naoardonate_sk\" value=\"Skrill\"   ".(strpos($setting[\'value\'],\'Skrill\') !== false? "checked=\"checked\"" : "" ) . "> Skrill <a href=\"https://www.moneybookers.com/app/?rid=19686949\" title=\"'
...
'),
		'value' => $naoardonate_payment_method,
		'disporder' => 1,
		'gid' => $gid
	);
    ...
    if($mybb->settings['naoardonate_ebank_ap'])
	    {
	...
	$settingsarray[] = array(
		...
		'optionscode' => 'text',
		'value' => $naoardonate_payment_method_pz,
		'disporder' => 2,
		'gid' => $gid
	);


    if($mybb->settings['naoardonate_ebank_lr'])
	    {
		$naoardonate_payment_method_lr = $mybb->settings['naoardonate_ebank_lr'];
	    }
	    elseif ($mybb->settings['teradonate_ebank_lr']) {
		$naoardonate_payment_method_lr = $mybb->settings['teradonate_ebank_lr'];

	    }
	    else {
		$naoardonate_payment_method_lr = '';
	    }



	$settingsarray[] = array(
		'name' => 'naoardonate_payment_method_lr',
		'title' => $db->escape_string($lang->naoardonate_settings_payment_method_LR),
		'description' => $db->escape_string($lang->naoardonate_settings_payment_method_LR_desc),
		'optionscode' => 'text',
		'value' => $naoardonate_payment_method_lr,
		'disporder' => 3,
		'gid' => $gid
	);

    if($mybb->settings['naoardonate_ebank_sk'])
	    {
	...
	$settingsarray[] = array(
		...
		'optionscode' => 'text',
		'value' => $naoardonate_payment_method_sk,
		'disporder' => 4,
		'gid' => $gid
	);


    if($mybb->settings['naoardonate_ebank_pp'])
	    {
	...
	$settingsarray[] = array(
		...
		'optionscode' => 'text',
		'value' => $naoardonate_payment_method_pp,
		'disporder' => 5,
		'gid' => $gid
	);
...
<input type=\"text\" size=\"7\" maxlength=\"3\" name=\"upsetting[{$setting[name]}]\" value=\"" . ((($v =($setting[\'value\']-time())/86400) <= 0 ) ? 0: round($v) ) ."\" /> Days'),
	...
	$settingsarray[] = array(
		...
		'optionscode' => 'textarea',
		'value' => $naoardonate_recievedmsg_100,
		'disporder' => 18,
		'gid' => $gid
	);
...
<optgroup label=\"' . $lang->naoardonate_global_currency_pz_sk_pp_wu_bk . '\">
...
",
	...
	$settingsarray[] = array(
		...
		'optionscode' => "yesno",
		'value' => $naoardonate_info_required,
		'disporder' => 22,
		'gid' => $gid
	);
...
" . $naoardonate_blockedgroups . "'),
		'value' => $naoardonate_cannotviewtop ,
		'disporder' => 27,
		'gid' => $gid
	);
	...
	$settingsarray[] = array(
		...
		'optionscode' => "textarea",
		'value' => $naoardonate_googleanalytics,
		'disporder' => 28,
		'gid' => $gid
	);
	...
	$settingsarray[] = array(
		'name' => 'naoardonate_copyleft',
		'title' => '',
		'description' => $db->escape_string('<h3 style="color:red">Copyright notice removal</h3><span style="color:#9900FF;font-size:small">If you do not like to show my copyright notice in the footer of your forum, please contact me <a href="https://red.coderme.com/mybb-donation-plugin" title="new page">this page</a></span>
<h3 style="color:orange">Premium plugin</h3><span style="color:#9900FF;font-size:small">Would you like if there is a premium version of this plugin?!, then contact me <a href="https://red.coderme.com/mybb-donation-plugin" title="new page">this page</a> and share your thoughts</span>
<h3 style="color:blue">Support</h3><span style="color:#9900FF;font-size:small">Thank you for using my plugin, I hope you liked it, <br />Well. coding this plugin was fun, however maintaning it is boring. If you like to see this plugin maintained and improved please consider making a small donation so I can dedicate more time for support and future improvement of this plugin, your donation is really appreciated.<br /><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=TASTPZE7LQ7HS&lc=GB&item_name=Support&button_subtype=services&no_note=1&no_shipping=1&rm=1&return=http%3a%2f%2fnaoar%2ecom&cancel_return=http%3a%2f%2fnaoar%2ecom&currency_code=USD&bn=PP%2dBuyNowBF%3abtn_donateCC_LG%2egif%3aNonHosted" title="new page" target="_blank"><img src="https://www.paypal.com/en_US/i/btn/btn_donateCC_LG.gif" /></a></span>'),
		'optionscode' => 'php',
		'value' => '',
		'disporder' => 29,
		'gid' => $gid
...
{
	global $db;
	$query = $db->simple_select('settings', 'name', "name='naoardonate_copyleft'");
	if($db->num_rows($query) > 0):
		return True;
	else:
		return False;
...
<br class="clear" />'),
		...
		$templates_array[] = array(
		'title' => 'naoardonate_links_donate_v3',
		...
		$templates_array[] = array(
		'title' => 'naoardonate_links_topdonors_v3',
		...
		$templates_array[] = array(
		'title' => 'naoardonate_img_topdonors_v3',
		...
		$templates_array[] = array(
		'title' => 'naoardonate_bar_tailtip_v3',
	...
	$templates_array[] = array(
		'title' => 'naoardonate_donate_aboutu_v3',
...
<table cellspacing="0" cellpadding="{$theme[\'tablespace\']}" class="w100">
	<tr>
		...
		$templates_array[] = array(
		'title' => 'naoardonate_donate_currencies_row_v3',
		...
		$templates_array[] = array(
		'title' => 'naoardonate_donate_offline_v3',
...
<table cellspacing="0" cellpadding="{$theme[\'tablespace\']}" class="w100">
	...
	{$payment_offline}
		...
		$templates_array[] = array(
		'title' => 'naoardonate_donate_note_v3',
		'template' => $db->escape_string('<fieldset class="w50 tleft">
...
<table cellspacing="0" cellpadding="{$theme[\'tablespace\']}" class="w100">
	...
	<noscript><textarea cols="33" rows="5" name="note" class="w100">{$note}</textarea></noscript>
		...
		$templates_array[] = array(
		'title' => 'naoardonate_donate_captcha_v3',
		'template' => $db->escape_string('<fieldset class="w50 tleft" >
...
{$footer}
	...
	$templates_array[] = array(
		'title' => 'naoardonate_top_donation_v3',
		'template' => $db->escape_string('<tr>
...
function naoar_showdonatelinks()
{
	...
	if($mybb->user['usergroup'] == 4 and $unconfirmed_donors > 0 and $mybb->settings['naoardonate_alert'] == 'notice')
	{
		...
		$pathtoadmin = $mybb->settings['bburl'] . '/' . $mybb->config['admin_dir'] . '/index.php?module=naoar_donors' . $sep . 'browse&amp;action=unconfirmed';
			eval("\$naoardonate_notice = \"" . $templates->get('naoardonate_notice_v3') . "\";");
		}
	}
	...
	if(!in_array($mybb->user['usergroup'], $naoardonate_from) or !$db->table_exists('naoardonate') or $mybb->settings['naoardonate_onoff'] == 0 or (!$mybb->settings['naoardonate_payment_method_pz'] and !$mybb->settings['naoardonate_payment_method_lr'] and !$mybb->settings['naoardonate_payment_method_sk'] and !$mybb->settings['naoardonate_payment_method_pp']) or strlen($mybb->settings['naoardonate_payment_method']) < 5) return; # yeah better now than later ..b
	$amount = intval($cache->read('naoardonate_goal'));
	eval('$naoardonate_donatelink = "'. $templates->get('naoardonate_links_donate_v3') . '";');
	$blocked_groups = explode(',',$mybb->settings['naoardonate_cannotviewtop']);

	if(!in_array($mybb->user['usergroup'],$blocked_groups))
	{
		eval('$naoardonate_donatelink .= "' . $templates->get('naoardonate_links_topdonors_v3') . '";');
	}
	...
	{
		if($mybb->settings['naoardonate_reason']){
			eval('$naoardonate_reason = "' . $templates->get('naoardonate_reason_v3'). '";');
		}
		if(!in_array($mybb->user['usergroup'],$blocked_groups)){
			eval('$naoardonate_top = "' . $templates->get('naoardonate_img_topdonors_v3') . '";');
		}
		...
		if ($amount >= (int)$mybb->settings['naoardonate_target'])
		{
			...
			eval('$tail_tip = "' . $templates->get('naoardonate_bar_tailtip_v3') . '";');
		...
		else
		{
			...
			eval('$tip_tail = "' . $templates->get('naoardonate_tiptail_v3') . '";');
			...
			eval('$left_div = "' . $templates->get('naoardonate_leftdiv_v3') . '";');
		}
		eval("\$naoardonate_bar = \"".$templates->get('naoardonate_bar_v3')."\";");
	}
	...
	if(stripos($_SERVER['SCRIPT_NAME'],'donate.php') !== false or $naoardonate_bar)
	{
		...
		$naoar_copyright = 'Donation\'s plugin by <a href="https://coderme.com" target="_blank">CoderMe.com</a>';
	}
